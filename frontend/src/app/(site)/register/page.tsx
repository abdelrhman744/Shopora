...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-20 relative overflow-hidden">
      <div className="absolute top-0 left-1/4 w-96 h-96 rounded-full blur-3xl opacity-20" style={{ background: "radial-gradient(circle, #FF2D95, transparent)" }} />
      <div className="absolute bottom-0 right-1/4 w-96 h-96 rounded-full blur-3xl opacity-20" style={{ background: "radial-gradient(circle, #FF8A00, transparent)" }} />
      <div className="relative w-full max-w-md">
        ...
        <div className="p-8 rounded-3xl" style={{ background: "#1A1C27", border: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <p className="text-sm text-[#8D93A5] mb-8">Join thousands of happy customers</p>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-xs text-[#8D93A5] mb-1.5 block">Full Name</label>
              <div className="relative">
                <UserIcon size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#8D93A5]" />
                <input
                  required
                  value={name}
          ...
          <p className="text-center text-sm text-[#8D93A5] mt-6">
            ...
            <Link href="/login" className="font-semibold" style={{ color: "#FF8A00" }}>
              Sign in
            </Link>
          </p>
        </div>
